...
// Package classify_test contains tests for the classify grammar.
...
func Example() {
	...
	// Create the Lexer
	lexer := classify.NewclassifyLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := classify.NewclassifyParser(stream)
...
func TestClassifyLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := classify.NewclassifyLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewclassifyLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestClassifyParser(t *testing.T) {
