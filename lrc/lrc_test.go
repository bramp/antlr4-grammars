...
// Package lrc_test contains tests for the lrc grammar.
...
var examples = []string{
	"grammars-v4/lrc/examples/example1.txt",
}

type exampleListener struct {
	*lrc.BaselrcParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := lrc.NewlrcLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := lrc.NewlrcParser(stream)
...
func TestLrcLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewlrcLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
