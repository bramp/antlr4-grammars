...
// Package teal_test contains tests for the Teal grammar.
...
var examples = []string{
	"grammars-v4/teal/examples/arrays.tl",
	"grammars-v4/teal/examples/enums.tl",
	"grammars-v4/teal/examples/functions.tl",
}

type exampleListener struct {
	*teal.BaseTealListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := teal.NewTealLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := teal.NewTealParser(stream)
...
func TestTealLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewTealLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestTealParser(t *testing.T) {
