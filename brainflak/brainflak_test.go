...
// Package brainflak_test contains tests for the brainflak grammar.
...
//
package brainflak_test

import (
	"bramp.net/antlr4/brainflak"
...
var examples = []string{
	"grammars-v4/brainflak/examples/example1.txt",
	"grammars-v4/brainflak/examples/example2.txt",
	"grammars-v4/brainflak/examples/example3.txt",
}

type exampleListener struct {
	*brainflak.BasebrainflakListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := brainflak.NewbrainflakLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := brainflak.NewbrainflakParser(stream)
...
func TestBrainflakLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := brainflak.NewbrainflakLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewbrainflakLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestBrainflakParser(t *testing.T) {
