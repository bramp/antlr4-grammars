...
// Package qif_test contains tests for the qif grammar.
...
var examples = []string{
	"grammars-v4/qif/examples/example1.txt",
	"grammars-v4/qif/examples/example2.txt",
}

type exampleListener struct {
	*qif.BaseqifParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := qif.NewqifLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := qif.NewqifParser(stream)
...
func TestQifLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewqifLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestQifParser(t *testing.T) {
