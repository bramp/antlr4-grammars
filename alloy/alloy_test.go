...
// Package alloy_test contains tests for the alloy grammar.
...
var examples = []string{
	"grammars-v4/alloy/examples/fact.als",
	"grammars-v4/alloy/examples/fs.als",
	"grammars-v4/alloy/examples/sig.als",
}

type exampleListener struct {
	*alloy.BasealloyListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := alloy.NewalloyLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := alloy.NewalloyParser(stream)
...
func TestAlloyLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := alloy.NewalloyLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewalloyLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestAlloyParser(t *testing.T) {
