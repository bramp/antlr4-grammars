...
// Package bsl_test contains tests for the BSL grammar.
...
var examples = []string{
	"grammars-v4/racket-bsl/examples/temperature-conversion.rkt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := bsl.NewBSLLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := bsl.NewBSLParser(stream)
...
func TestBSLLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewBSLLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestBSLParser(t *testing.T) {
