...
// Package apt_test contains tests for the apt grammar.
...
func Example() {
	...
	// Create the Lexer
	lexer := apt.NewaptLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := apt.NewaptParser(stream)
...
func TestAptLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewaptLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
