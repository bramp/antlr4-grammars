...
// Package sici_test contains tests for the sici grammar.
...
var examples = []string{
	"grammars-v4/sici/examples/example1.txt",
	"grammars-v4/sici/examples/example2.txt",
}

type exampleListener struct {
	*sici.BasesiciListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := sici.NewsiciLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := sici.NewsiciParser(stream)
...
func TestSiciLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewsiciLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestSiciParser(t *testing.T) {
