...
// Package verilog_test contains tests for the Verilog grammar.
...
var examples = []string{
	"grammars-v4/verilog/verilog/examples/compiler_directives.v",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := verilog.NewVerilogLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := verilog.NewVerilogParser(stream)
...
func TestVerilogLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewVerilogLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestVerilogParser(t *testing.T) {
