...
// Package asmz80_test contains tests for the asmZ80 grammar.
...
var examples = []string{
	"grammars-v4/asm/asmZ80/examples/CPM22.Z80",
}

type exampleListener struct {
	*asmz80.BaseasmZ80Listener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := asmz80.NewasmZ80Lexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := asmz80.NewasmZ80Parser(stream)
...
func TestAsmZ80Lexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := asmz80.NewasmZ80Lexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewasmZ80Lexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestAsmZ80Parser(t *testing.T) {
