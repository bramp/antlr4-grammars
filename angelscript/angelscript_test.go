...
// Package angelscript_test contains tests for the angelscript grammar.
...
var examples = []string{
	"grammars-v4/angelscript/examples/example1.txt",
}

type exampleListener struct {
	*angelscript.BaseangelscriptListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := angelscript.NewangelscriptLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := angelscript.NewangelscriptParser(stream)
...
func TestAngelscriptLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := angelscript.NewangelscriptLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewangelscriptLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestAngelscriptParser(t *testing.T) {
