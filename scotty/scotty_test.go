...
// Package scotty_test contains tests for the scotty grammar.
...
var examples = []string{
	"grammars-v4/scotty/examples/example1.txt",
}

type exampleListener struct {
	*scotty.BasescottyListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := scotty.NewscottyLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := scotty.NewscottyParser(stream)
...
func TestScottyLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := scotty.NewscottyLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewscottyLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestScottyParser(t *testing.T) {
