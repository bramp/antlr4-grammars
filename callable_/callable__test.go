...
// Package callable__test contains tests for the callable_ grammar.
...
//
package callable__test

import (
	"bramp.net/antlr4/callable_"
...
var examples = []string{
	"grammars-v4/callable/examples/example1.txt",
	"grammars-v4/callable/examples/example2.txt",
	"grammars-v4/callable/examples/example3.txt",
}

type exampleListener struct {
	*callable_.Basecallable_Listener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := callable_.Newcallable_Lexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := callable_.Newcallable_Parser(stream)
...
func TestCallable_Lexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := callable_.Newcallable_Lexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("Newcallable_Lexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestCallable_Parser(t *testing.T) {
