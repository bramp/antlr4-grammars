...
// Package refal_test contains tests for the refal grammar.
...
var examples = []string{
	"grammars-v4/refal/examples/example1.txt",
}

type exampleListener struct {
	*refal.BaserefalListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := refal.NewrefalLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := refal.NewrefalParser(stream)
...
func TestRefalLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := refal.NewrefalLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewrefalLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestRefalParser(t *testing.T) {
