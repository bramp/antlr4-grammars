...
// Package orwell_test contains tests for the orwell grammar.
...
var examples = []string{
	"grammars-v4/orwell/examples/example1.txt",
	"grammars-v4/orwell/examples/example2.txt",
	"grammars-v4/orwell/examples/example3.txt",
}

type exampleListener struct {
	*orwell.BaseorwellListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := orwell.NeworwellLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := orwell.NeworwellParser(stream)
...
func TestOrwellLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := orwell.NeworwellLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NeworwellLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestOrwellParser(t *testing.T) {
