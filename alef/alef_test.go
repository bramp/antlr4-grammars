...
// Package alef_test contains tests for the alef grammar.
...
var examples = []string{
	"grammars-v4/alef/examples/example1.txt",
}

type exampleListener struct {
	*alef.BasealefListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := alef.NewalefLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := alef.NewalefParser(stream)
...
func TestAlefLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewalefLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestAlefParser(t *testing.T) {
