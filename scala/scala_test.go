...
// Package scala_test contains tests for the Scala grammar.
...
var examples = []string{
	"grammars-v4/scala/examples/class.scala",
	"grammars-v4/scala/examples/class2.scala",
	"grammars-v4/scala/examples/example1.scala",
	"grammars-v4/scala/examples/example2.scala",
	"grammars-v4/scala/examples/object.scala",
	"grammars-v4/scala/examples/string.scala",
}

type exampleListener struct {
	*scala.BaseScalaListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := scala.NewScalaLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := scala.NewScalaParser(stream)
...
func TestScalaLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := scala.NewScalaLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewScalaLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
