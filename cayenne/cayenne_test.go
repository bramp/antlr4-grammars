...
// Package cayenne_test contains tests for the cayenne grammar.
...
//
package cayenne_test

import (
	"bramp.net/antlr4/cayenne"
...
func Example() {
	...
	// Create the Lexer
	lexer := cayenne.NewcayenneLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := cayenne.NewcayenneParser(stream)
...
func TestCayenneLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := cayenne.NewcayenneLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewcayenneLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestCayenneParser(t *testing.T) {
