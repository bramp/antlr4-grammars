...
// Package karel_test contains tests for the karel grammar.
...
var examples = []string{
	"grammars-v4/karel/examples/example1.txt",
}

type exampleListener struct {
	*karel.BasekarelListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := karel.NewkarelLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := karel.NewkarelParser(stream)
...
func TestKarelLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := karel.NewkarelLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewkarelLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestKarelParser(t *testing.T) {
