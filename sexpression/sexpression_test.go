// Package sexpression_test contains tests for the sexpression grammar.
...
import (
	"bramp.net/antlr4test-go/sexpression"
...
var examples = []string{
	"grammars-v4/sexpression/examples/example1.txt",
	"grammars-v4/sexpression/examples/example2.txt",
}
...
func TestsexpressionLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := sexpression.NewsexpressionLexer(input)
...
func TestsexpressionParser(t *testing.T) {
	for _, file := range examples {
		input, err := newCharStream(file)
		if err != nil {
			t.Errorf("Failed to open example file: %s", err)
