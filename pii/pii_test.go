...
// Package pii_test contains tests for the pii grammar.
...
var examples = []string{
	"grammars-v4/pii/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := pii.NewpiiLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := pii.NewpiiParser(stream)
...
func TestPiiLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewpiiLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestPiiParser(t *testing.T) {
