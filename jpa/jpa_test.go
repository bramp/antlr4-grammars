// Package jpa_test contains tests for the JPA grammar.
...
var examples = []string{
	"grammars-v4/jpa/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := jpa.NewJPALexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := jpa.NewJPAParser(stream)
...
func TestJPALexer(t *testing.T) {
	for _, file := range examples {
		...
		if i == MAX_TOKENS {
			t.Errorf("NewJPALexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestJPAParser(t *testing.T) {
