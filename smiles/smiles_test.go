...
// Package smiles_test contains tests for the smiles grammar.
...
func Example() {
	...
	// Create the Lexer
	lexer := smiles.NewsmilesLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := smiles.NewsmilesParser(stream)
...
func TestSmilesLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := smiles.NewsmilesLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewsmilesLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestSmilesParser(t *testing.T) {
