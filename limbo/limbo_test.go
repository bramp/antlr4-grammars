...
// Package limbo_test contains tests for the limbo grammar.
...
var examples = []string{
	"grammars-v4/limbo/examples/example1.txt",
}

type exampleListener struct {
	*limbo.BaselimboListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := limbo.NewlimboLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := limbo.NewlimboParser(stream)
...
func TestLimboLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := limbo.NewlimboLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewlimboLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestLimboParser(t *testing.T) {
