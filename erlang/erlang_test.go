...
// Package erlang_test contains tests for the Erlang grammar.
...
var examples = []string{
	"grammars-v4/erlang/examples/fac.P",
	"grammars-v4/erlang/examples/getty.P",
	"grammars-v4/erlang/examples/helloword.P",
	"grammars-v4/erlang/examples/map.P",
}

type exampleListener struct {
	*erlang.BaseErlangListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := erlang.NewErlangLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := erlang.NewErlangParser(stream)
...
func TestErlangLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := erlang.NewErlangLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewErlangLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestErlangParser(t *testing.T) {
