...
// Package lisa_test contains tests for the lisa grammar.
...
var examples = []string{
	"grammars-v4/lisa/examples/example1.txt",
}

type exampleListener struct {
	*lisa.BaselisaListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := lisa.NewlisaLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := lisa.NewlisaParser(stream)
...
func TestLisaLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewlisaLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestLisaParser(t *testing.T) {
