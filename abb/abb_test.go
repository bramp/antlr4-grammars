...
// Package abb_test contains tests for the abb grammar.
...
var examples = []string{
	"grammars-v4/abb/examples/robdata.sys",
}

type exampleListener struct {
	*abb.BaseabbParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := abb.NewabbLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := abb.NewabbParser(stream)
...
func TestAbbLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewabbLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestAbbParser(t *testing.T) {
