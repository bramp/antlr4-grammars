...
// Package isl_test contains tests for the ISL grammar.
...
var examples = []string{
	"grammars-v4/racket-isl/examples/list-contains.rkt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := isl.NewISLLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := isl.NewISLParser(stream)
...
func TestISLLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewISLLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestISLParser(t *testing.T) {
