...
// Package metamath_test contains tests for the metamath grammar.
...
var examples = []string{
	"grammars-v4/metamath/examples/example1.txt",
	"grammars-v4/metamath/examples/example2.txt",
}

type exampleListener struct {
	*metamath.BasemetamathListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := metamath.NewmetamathLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := metamath.NewmetamathParser(stream)
...
func TestMetamathLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := metamath.NewmetamathLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewmetamathLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestMetamathParser(t *testing.T) {
