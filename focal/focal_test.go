...
// Package focal_test contains tests for the focal grammar.
...
var examples = []string{
	"grammars-v4/focal/examples/example1.txt",
}

type exampleListener struct {
	*focal.BasefocalListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := focal.NewfocalLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := focal.NewfocalParser(stream)
...
func TestFocalLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := focal.NewfocalLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewfocalLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestFocalParser(t *testing.T) {
