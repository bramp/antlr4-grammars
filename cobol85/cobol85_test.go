// Package cobol85_test contains tests for the Cobol85 grammar.
...
//
package cobol85_test

import (
	"bramp.net/antlr4test-go/cobol85"
...
func TestCobol85Lexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := cobol85.NewCobol85Lexer(input)
		...
		if i == MAX_TOKENS {
			t.Errorf("NewCobol85Lexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestCobol85Parser(t *testing.T) {
	for _, file := range examples {
		input, err := newCharStream(file)
		if err != nil {
			t.Errorf("Failed to open example file: %s", err)
