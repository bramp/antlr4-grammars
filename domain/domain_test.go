...
// Package domain_test contains tests for the domain grammar.
...
var examples = []string{
	"grammars-v4/rfc1035/examples/example1.txt",
	"grammars-v4/rfc1035/examples/example2.txt",
	"grammars-v4/rfc1035/examples/example3.txt",
}

type exampleListener struct {
	*domain.BasedomainListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := domain.NewdomainLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := domain.NewdomainParser(stream)
...
func TestDomainLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := domain.NewdomainLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewdomainLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
