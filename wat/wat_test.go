...
// Package wat_test contains tests for the Wat grammar.
...
var examples = []string{
	"grammars-v4/wat/examples/shared1.wat",
}

type exampleListener struct {
	*wat.BaseWatParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := wat.NewWatLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := wat.NewWatParser(stream)
...
func TestWatLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewWatLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestWatParser(t *testing.T) {
