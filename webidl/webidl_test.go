...
// Package webidl_test contains tests for the WebIDL grammar.
...
var examples = []string{
	"grammars-v4/webidl/examples/document.idl",
	"grammars-v4/webidl/examples/html_link_element.idl",
}

type exampleListener struct {
	*webidl.BaseWebIDLListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := webidl.NewWebIDLLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := webidl.NewWebIDLParser(stream)
...
func TestWebIDLLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := webidl.NewWebIDLLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewWebIDLLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestWebIDLParser(t *testing.T) {
