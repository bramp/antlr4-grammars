// Package muparser_test contains tests for the MuParser grammar.
...
//
package muparser_test

import (
	"bramp.net/antlr4-grammars/muparser"
	"fmt"
...
var examples = []string{
	"grammars-v4/muparser/examples/example1.txt",
}

type exampleListener struct {
	*muparser.BaseMuParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := muparser.NewMuParserLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := muparser.NewMuParserParser(stream)
...
func TestMuParserLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := muparser.NewMuParserLexer(input)
		...
		if i == MAX_TOKENS {
			t.Errorf("NewMuParserLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestMuParserParser(t *testing.T) {
