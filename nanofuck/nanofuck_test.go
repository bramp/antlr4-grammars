...
// Package nanofuck_test contains tests for the nanofuck grammar.
...
//
package nanofuck_test

import (
	"bramp.net/antlr4/internal"
	"bramp.net/antlr4/nanofuck"
...
var examples = []string{
	"grammars-v4/nanofuck/examples/example1.txt",
	"grammars-v4/nanofuck/examples/example2.txt",
}

type exampleListener struct {
	*nanofuck.BasenanofuckListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := nanofuck.NewnanofuckLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := nanofuck.NewnanofuckParser(stream)
...
func TestNanofuckLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := nanofuck.NewnanofuckLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewnanofuckLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestNanofuckParser(t *testing.T) {
