...
// Package infosapient_test contains tests for the infosapient grammar.
...
//
package infosapient_test

import (
	"bramp.net/antlr4/infosapient"
...
type exampleListener struct {
	*infosapient.BaseinfosapientListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := infosapient.NewinfosapientLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := infosapient.NewinfosapientParser(stream)
	p.BuildParseTrees = true
	p.AddErrorListener(antlr.NewDiagnosticErrorListener(true))

	// Finally walk the tree
	tree := p.ConditionalRule()
...
func TestInfosapientLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := infosapient.NewinfosapientLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewinfosapientLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestInfosapientParser(t *testing.T) {
