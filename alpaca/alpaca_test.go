...
// Package alpaca_test contains tests for the alpaca grammar.
...
var examples = []string{
	"grammars-v4/alpaca/examples/conway.alp",
}

type exampleListener struct {
	*alpaca.BasealpacaListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := alpaca.NewalpacaLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := alpaca.NewalpacaParser(stream)
...
func TestAlpacaLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := alpaca.NewalpacaLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewalpacaLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestAlpacaParser(t *testing.T) {
