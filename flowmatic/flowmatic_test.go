...
// Package flowmatic_test contains tests for the flowmatic grammar.
...
var examples = []string{
	"grammars-v4/flowmatic/examples/example1.txt",
}

type exampleListener struct {
	*flowmatic.BaseflowmaticListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := flowmatic.NewflowmaticLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := flowmatic.NewflowmaticParser(stream)
...
func TestFlowmaticLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := flowmatic.NewflowmaticLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewflowmaticLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestFlowmaticParser(t *testing.T) {
