...
// Package ctl_test contains tests for the ctl grammar.
...
var examples = []string{
	"grammars-v4/ctl/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := ctl.NewctlLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := ctl.NewctlParser(stream)
...
func TestCtlLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewctlLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestCtlParser(t *testing.T) {
