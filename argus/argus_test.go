...
// Package argus_test contains tests for the argus grammar.
...
func Example() {
	...
	// Create the Lexer
	lexer := argus.NewargusLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := argus.NewargusParser(stream)
...
func TestArgusLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := argus.NewargusLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewargusLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestArgusParser(t *testing.T) {
