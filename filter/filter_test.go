...
// Package filter_test contains tests for the filter grammar.
...
var examples = []string{
	"grammars-v4/rfc1960/examples/example1.txt",
	"grammars-v4/rfc1960/examples/example2.txt",
	"grammars-v4/rfc1960/examples/example3.txt",
	"grammars-v4/rfc1960/examples/example4.txt",
}

type exampleListener struct {
	*filter.BasefilterListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := filter.NewfilterLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := filter.NewfilterParser(stream)
...
func TestFilterLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := filter.NewfilterLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewfilterLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
