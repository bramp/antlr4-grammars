...
// Package powerquery_test contains tests for the PowerQuery grammar.
...
//
package powerquery_test

import (
	"bramp.net/antlr4/internal"
	"bramp.net/antlr4/powerquery"
...
type exampleListener struct {
	*powerquery.BasePowerQueryParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := powerquery.NewPowerQueryLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := powerquery.NewPowerQueryParser(stream)
...
func TestPowerQueryLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := powerquery.NewPowerQueryLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewPowerQueryLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestPowerQueryParser(t *testing.T) {
