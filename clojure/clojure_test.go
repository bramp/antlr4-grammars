...
// Package clojure_test contains tests for the Clojure grammar.
...
var examples = []string{
	"grammars-v4/clojure/examples/example1.txt",
}

type exampleListener struct {
	*clojure.BaseClojureListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := clojure.NewClojureLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := clojure.NewClojureParser(stream)
...
func TestClojureLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := clojure.NewClojureLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewClojureLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestClojureParser(t *testing.T) {
