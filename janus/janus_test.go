...
// Package janus_test contains tests for the janus grammar.
...
var examples = []string{
	"grammars-v4/janus/examples/example1.txt",
}

type exampleListener struct {
	*janus.BasejanusListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := janus.NewjanusLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := janus.NewjanusParser(stream)
...
func TestJanusLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := janus.NewjanusLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewjanusLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestJanusParser(t *testing.T) {
