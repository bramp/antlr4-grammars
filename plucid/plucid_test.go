...
// Package plucid_test contains tests for the plucid grammar.
...
func Example() {
	...
	// Create the Lexer
	lexer := plucid.NewplucidLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := plucid.NewplucidParser(stream)
...
func TestPlucidLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := plucid.NewplucidLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewplucidLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestPlucidParser(t *testing.T) {
