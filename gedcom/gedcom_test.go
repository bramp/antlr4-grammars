...
// Package gedcom_test contains tests for the gedcom grammar.
...
var examples = []string{
	"grammars-v4/gedcom/examples/example1.txt",
}

type exampleListener struct {
	*gedcom.BasegedcomListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := gedcom.NewgedcomLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := gedcom.NewgedcomParser(stream)
...
func TestGedcomLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := gedcom.NewgedcomLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewgedcomLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestGedcomParser(t *testing.T) {
