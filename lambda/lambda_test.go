...
// Package lambda_test contains tests for the lambda grammar.
...
var examples = []string{
	"grammars-v4/lambda/examples/example1.txt",
	"grammars-v4/lambda/examples/example2.txt",
	"grammars-v4/lambda/examples/example3.txt",
	"grammars-v4/lambda/examples/example4.txt",
...
func TestLambdaLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := lambda.NewlambdaLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewlambdaLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
