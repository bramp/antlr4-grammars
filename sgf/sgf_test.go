...
// Package sgf_test contains tests for the sgf grammar.
...
var examples = []string{
	"grammars-v4/sgf/examples/8718-KuroNeko-creedfoxx-chensicha.sgf",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := sgf.NewsgfLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := sgf.NewsgfParser(stream)
...
func TestSgfLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewsgfLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
