...
// Package itn_test contains tests for the itn grammar.
...
var examples = []string{
	"grammars-v4/itn/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := itn.NewitnLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := itn.NewitnParser(stream)
...
func TestItnLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewitnLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestItnParser(t *testing.T) {
