...
// Package bibcode_test contains tests for the bibcode grammar.
...
//
package bibcode_test

import (
	"bramp.net/antlr4/bibcode"
...
var examples = []string{
	"grammars-v4/bibcode/examples/example1.txt",
	"grammars-v4/bibcode/examples/example2.txt",
	"grammars-v4/bibcode/examples/example3.txt",
	"grammars-v4/bibcode/examples/example4.txt",
}

type exampleListener struct {
	*bibcode.BasebibcodeListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := bibcode.NewbibcodeLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := bibcode.NewbibcodeParser(stream)
...
func TestBibcodeLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := bibcode.NewbibcodeLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewbibcodeLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestBibcodeParser(t *testing.T) {
