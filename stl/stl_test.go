...
// Package stl_test contains tests for the STL grammar.
...
func Example() {
	...
	// Create the Lexer
	lexer := stl.NewSTLLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := stl.NewSTLParser(stream)
...
func TestSTLLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewSTLLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
