...
// Package snowball_test contains tests for the snowball grammar.
...
var examples = []string{
	"grammars-v4/snowball/examples/example1.txt",
}

type exampleListener struct {
	*snowball.BasesnowballListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := snowball.NewsnowballLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := snowball.NewsnowballParser(stream)
...
func TestSnowballLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := snowball.NewsnowballLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewsnowballLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestSnowballParser(t *testing.T) {
