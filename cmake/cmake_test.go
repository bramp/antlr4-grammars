...
// Package cmake_test contains tests for the CMake grammar.
...
var examples = []string{
	"grammars-v4/cmake/examples/CMakeLists.txt",
}

type exampleListener struct {
	*cmake.BaseCMakeListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := cmake.NewCMakeLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := cmake.NewCMakeParser(stream)
...
func TestCMakeLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := cmake.NewCMakeLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewCMakeLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestCMakeParser(t *testing.T) {
