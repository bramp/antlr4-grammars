...
// Package vmf_test contains tests for the vmf grammar.
...
var examples = []string{
	"grammars-v4/vmf/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := vmf.NewvmfLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := vmf.NewvmfParser(stream)
...
func TestVmfLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewvmfLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestVmfParser(t *testing.T) {
