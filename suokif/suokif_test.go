...
// Package suokif_test contains tests for the SUOKIF grammar.
...
var examples = []string{
	"grammars-v4/suokif/examples/example1.txt",
...
func TestSUOKIFLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := suokif.NewSUOKIFLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewSUOKIFLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}
