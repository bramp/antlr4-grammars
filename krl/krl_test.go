...
// Package krl_test contains tests for the krl grammar.
...
var examples = []string{
	"grammars-v4/kuka/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := krl.NewkrlLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := krl.NewkrlParser(stream)
...
func TestKrlLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewkrlLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestKrlParser(t *testing.T) {
