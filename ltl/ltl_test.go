...
// Package ltl_test contains tests for the ltl grammar.
...
var examples = []string{
	"grammars-v4/ltl/examples/example1.txt",
}
...
func Example() {
	...
	// Create the Lexer
	lexer := ltl.NewltlLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := ltl.NewltlParser(stream)
...
func TestLtlLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewltlLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestLtlParser(t *testing.T) {
