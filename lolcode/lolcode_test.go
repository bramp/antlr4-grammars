...
// Package lolcode_test contains tests for the lolcode grammar.
...
//
package lolcode_test

import (
	"bramp.net/antlr4/internal"
	"bramp.net/antlr4/lolcode"
...
var examples = []string{
	"grammars-v4/lolcode/examples/hai.txt",
}

type exampleListener struct {
	*lolcode.BaselolcodeListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := lolcode.NewlolcodeLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := lolcode.NewlolcodeParser(stream)
...
func TestLolcodeLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := lolcode.NewlolcodeLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewlolcodeLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestLolcodeParser(t *testing.T) {
