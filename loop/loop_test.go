...
// Package loop_test contains tests for the loop grammar.
...
var examples = []string{
	"grammars-v4/loop/examples/example1.txt",
	"grammars-v4/loop/examples/example2.txt",
	"grammars-v4/loop/examples/example5.txt",
}

type exampleListener struct {
	*loop.BaseloopListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := loop.NewloopLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := loop.NewloopParser(stream)
...
func TestLoopLexer(t *testing.T) {
	for _, file := range examples {
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewloopLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestLoopParser(t *testing.T) {
