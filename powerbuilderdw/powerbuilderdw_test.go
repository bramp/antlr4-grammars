...
// Package powerbuilderdw_test contains tests for the PowerBuilderDW grammar.
...
//
package powerbuilderdw_test

import (
	"bramp.net/antlr4/internal"
	"bramp.net/antlr4/powerbuilderdw"
...
type exampleListener struct {
	*powerbuilderdw.BasePowerBuilderDWParserListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := powerbuilderdw.NewPowerBuilderDWLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := powerbuilderdw.NewPowerBuilderDWParser(stream)
...
func TestPowerBuilderDWLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := powerbuilderdw.NewPowerBuilderDWLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewPowerBuilderDWLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestPowerBuilderDWParser(t *testing.T) {
