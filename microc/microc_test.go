...
// Package microc_test contains tests for the microc grammar.
...
var examples = []string{
	"grammars-v4/microc/examples/example1.c",
	"grammars-v4/microc/examples/example2.c",
	"grammars-v4/microc/examples/example5.c",
}

type exampleListener struct {
	*microc.BasemicrocListener
}
...
func Example() {
	...
	// Create the Lexer
	lexer := microc.NewmicrocLexer(is)
	stream := antlr.NewCommonTokenStream(lexer, antlr.TokenDefaultChannel)

	// Create the Parser
	p := microc.NewmicrocParser(stream)
...
func TestMicrocLexer(t *testing.T) {
	for _, file := range examples {
		...
		// Create the Lexer
		lexer := microc.NewmicrocLexer(input)
		...
		if i >= MAX_TOKENS {
			t.Errorf("NewmicrocLexer(%q) read %d tokens without finding EOF", file, i)
		}
	}
}

func TestMicrocParser(t *testing.T) {
